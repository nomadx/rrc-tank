...
void DemoPixel::Pause() {
	isEntered = false;
}
void DemoPixel::Resume() {
	isEntered = true;
	enteredTime = 0.0f;

	char str[30];
	sprintf(str, "entered DemoPixel state.");
	glfwSetWindowTitle(str);

	APP.SetCursorVisible(false);
}

void DemoPixel::Update() {
	if (isEntered) {
